    ...
    namespace Composites{
        ...
        class baseGridModel{
            ...
            public:
                ...
                template<class GO, class GO_EXT, class GO_OVLP, class V, class GFS, class GFS_EXT, class C, class CON, class MBE, class LOP>
                    bool inline solve(GO& go, GO_EXT& go_ext, GO_OVLP& go_ovlp, V& u, const GFS& gfs, const GFS_EXT& gfs_ext, C& cg, CON& constraints, MBE& mbe, LOP& lop){
                        ...
                        if(solverParameters.UG == true){
                            ...
                            if(gfs.gridView().comm().size() > 1){ // == DEFAULT AVAILABLE PARALLEL SOLVERS
                                if(solverParameters.preconditioner == "GenEO"){
                                    ...
                                    typedef Dune::Geneo::CG_GenEO<V,GFS,GFS_EXT,GO,GO_EXT,GO_OVLP,LOP,CON,MBE> GenEO;
                                    int cells = std::pow(gfs.gridView().size(0),1./3.);
                                    GenEO mysolver(u,go_ext,go_ovlp,gfs,gfs_ext,lop,constraints,mbe,solverParameters,helper, cells);
				    typedef Dune::PDELab::NewtonMethod<GO,GenEO> Newton;
				    Newton newton(go,mysolver);
                                    //mysolver.apply();
  				    newton.apply(u);
                                    return flag;
#else
                                ...
                                if(solverParameters.preconditioner == "OneLevel_AdditiveSchwarz"){
                                    ...
                                    //slp.apply();
                                    typedef Dune::PDELab::NewtonMethod<GO,PAR_UMF> Newton;
				    Newton newton(go,ls);
				    newton.apply(u);
                                    return flag;
#else
